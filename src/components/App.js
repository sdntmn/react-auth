  ...
  function handleCardDelete(card) {
    api
      ...
      .then(() => {
        ...
        Тут в стейт-функцию передается стейт-колбэк, в котором 1м аргументом 
        идет текущее состояние переменной. Вот ее и нужно использовать (менять)
        при изменениях,  так как бывают ситуации, что где-то уже изменили эту
        переменную, но еще не обновились данные в ней, а Вы попытаетесь 
        изменить старые данные, которые неактуальны больше */
      })
      .catch((error) => {
